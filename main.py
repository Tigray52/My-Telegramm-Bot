...
class Storage:
    def __init__(self):
        self.questions = {}
        self.active_chats = {}
        self.banned_users = {}  # Изменено с set на dict: {user_id: {'until': timestamp, 'reason': str}}
        self.user_profiles = {}
        ...
        self.user_message_counts = {}  # Для антиспама: {user_id: {'count': int, 'reset_time': timestamp}}
        self.message_history = {}  # Для функции ответа: {user_id: [{'id': message_id, 'text': str, 'time': timestamp}]}
        self.load_data()
    ...
    def check_spam(self, user_id):
        ...
        if user_id not in self.user_message_counts:
            self.user_message_counts[user_id] = {
                'count': 1,
                'reset_time': now + 10  # Сброс через 10 секунд
            }
            self.save_data()
            return False
        ...
        # Если более 10 сообщений за 10 секунд - спам
    ...
    def add_to_message_history(self, user_id, message_id, text, is_admin=False):
        ...
        self.message_history[user_id].append({
            'id': message_id,
            'text': text[:200],  # Обрезаем длинные сообщения
            'time': time.time(),
            'is_admin': is_admin
        })
...
def parse_duration(duration_str):
    ...
    patterns = [
        (r'(\d+)\s*y', 365 * 24 * 3600),  # годы
        (r'(\d+)\s*mon', 30 * 24 * 3600),  # месяцы
        (r'(\d+)\s*w', 7 * 24 * 3600),     # недели
        (r'(\d+)\s*d', 24 * 3600),         # дни
        (r'(\d+)\s*h', 3600),              # часы
        (r'(\d+)\s*m', 60),                # минуты
        (r'(\d+)\s*s', 1)                  # секунды
...
def show_user_help(message):
    help_text = (
        ...
        "• Можно отвечать на сообщения (используйте кнопку 'Ответить' в Telegram)\n\n"
        "*💬 В чате:*\n"
...
def show_admin_help(message):
    help_text = (
        ...
        "`/ban 123456789 1y1d5h10s нарушение` - на 1 год 1 день 5 часов 10 секунд\n\n"
        ...
        "*Команда /full:*\n"
        "• `/full#1`, `/full #1` или `/full 1`\n"
        "• Или после нажатия кнопки 'Ответить'\n\n"
        
        "*Функция ответа в чате:*\n"
        "• Используйте кнопку 'Ответить' в Telegram\n"
        "• Сообщение будет переслано с указанием, на что отвечаете\n\n"
...
def end_chat(user_id, reason="normal"):
    ...
    if user_id in storage.active_chats:
        ...
        if reason != "ban" and not storage.is_banned(user_id):
            try:
...
def ban_command(message):
    ...
    if len(parts) >= 3:
        # Проверяем, является ли третий аргумент временем
...
# ===== ОБРАБОТКА СООБЩЕНИЙ С ФУНКЦИЕЙ ОТВЕТА =====
def handle_message_with_reply(message, is_user_message=True):
    """Обрабатывает сообщение с возможным ответом на другое сообщение"""
    user_id = message.from_user.id
    ...
    if reply_to_message_id:
        ...
        if is_user_message:
            # Пользователь отвечает в чате админу
            chat_data = storage.active_chats.get(user_id)
            if chat_data:
                admin_id = chat_data['admin_id']
                original_msg = storage.get_message_by_id(admin_id, reply_to_message_id)
        else:
            ...
            if active_user_id:
                original_msg = storage.get_message_by_id(active_user_id, reply_to_message_id)
        
        if original_msg:
            # Создаем текст с цитированием
            original_text_short = original_msg['text']
            if len(original_text_short) > 50:
                original_text_short = original_text_short[:47] + "..."
            
            reply_text = f"↪️ *Ответ на:* {original_text_short}\n\n"
...
def handle_all_messages(message):
    ...
    # Проверка на спам для всех пользователей
    if storage.check_spam(user_id):
        storage.ban_user(user_id, 3600, "Спам (более 10 сообщений за 10 секунд)")
        bot.send_message(
            user_id,
...
def handle_user_in_chat(message):
    ...
    try:
        ...
        # Обрабатываем возможный ответ на сообщение
        reply_header = handle_message_with_reply(message, is_user_message=True)
        ...
        if urls and not allow_links:
            ...
            if reply_header:
                admin_message += reply_header
            
            admin_message += masked_text
        ...
        admin_message = f"👤 *{sender}:*\n"
        
        if reply_header:
            admin_message += reply_header
        
        admin_message += text[:500]
...
def handle_admin_to_user(message):
    ...
    try:
        if message.content_type == 'text':
            # Обрабатываем возможный ответ на сообщение
            reply_header = handle_message_with_reply(message, is_user_message=False)
            
            user_message = f"👨‍💼 *{chat_data['admin_name']} (Администратор):*\n"
            
            if reply_header:
                user_message += reply_header
...
def show_bans(message):
    ...
    for user_id, ban_data in active_bans:
        ...
        text += f"  Причина: {ban_data['reason']}\n"
        text += f"  Бан: {duration}\n\n"
...
def handle_callbacks(call):
    ...
    if call.data.startswith('accept_chat_'):
        ...
        # Проверяем, не принят ли уже этот запрос
        if question.get('status') == 'accepted':
        ...
        if question.get('status') in ['accepted', 'rejected']:
            bot.answer_callback_query(call.id, "❌ Этот запрос уже был обработан")
            return
...
def complete_chat_setup(user_id, question_id, confirmation, allow_links, limit):
    ...
    bot.send_message(
        ...
        f"💬 *Можно отвечать на сообщения* (используйте кнопку 'Ответить')\n"
...
def process_ban_with_reason(message, user_id):
    ...
    if len(parts) == 1:
        # Только причина или только время
