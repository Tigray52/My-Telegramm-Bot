...
class Storage:
    def __init__(self):
        ...
        self.user_cooldowns = {}
        self.pending_chat_setup = {}  # {user_id: True} - ожидают настройки чата
        self.load_data()
        ...
        try:
            if os.path.exists('storage.json'):
                with open('storage.json', 'r', encoding='utf-8') as f:
                    ...
                    self.pending_chat_setup = data.get('pending_chat_setup', {})
        except:
            pass
    ...
    def save_data(self):
        data = {
            ...
            'pending_chat_setup': self.pending_chat_setup
        }
    ...
    try:
        ...
        if parsed.netloc:
            ...
            if '.' in domain:
                parts = domain.split('.')
                if len(parts) >= 2:
                    # Маскируем каждый символ до точки звездочками
                    masked_parts = []
                    for part in parts[:-1]:  # Все части кроме домена
                        if len(part) > 0:
...
def ban_command(message):
    ...
    # Убираем [] вокруг причины, если они есть
...
# ===== КОМАНДА /MESSAGE С РАМКАМИ ПО УМОЛЧАНИЮ FALSE =====
...
# ===== КОМАНДА /FULL ДЛЯ РАСКРЫТИЯ ССЫЛОК =====
...
def full_command(message):
    ...
    if not message.reply_to_message:
        bot.send_message(ADMIN_ID, "❌ Используйте эту команду как ответ на вопрос со скрытой ссылкой.")
        return
    
    reply_msg = message.reply_to_message
    question_id = None
    
    # Ищем вопрос по тексту сообщения
    for qid, question in storage.questions.items():
        if (question.get('has_media') and reply_msg.caption and question['text'] in reply_msg.caption) or \
           (not question.get('has_media') and reply_msg.text and question['text'] in reply_msg.text):
            question_id = qid
            break
    
    if not question_id:
        bot.send_message(ADMIN_ID, "❌ Не удалось найти вопрос.")
        return
    ...
    urls = re.findall(r'https?://[^\s<>"]+|www\.[^\s<>"]+', question['text'])
    if urls:
        full_text += f"\n\n🔗 *Ссылки:*\n"
        for i, url in enumerate(urls, 1):
            full_text += f"{i}. {url}\n"
    
    bot.send_message(ADMIN_ID, full_text, parse_mode='Markdown', disable_web_page_preview=False)
    
    # Просим отправить ответ
...
# ===== ОБРАБОТКА СООБЩЕНИЙ В ЧАТЕ (ИСПРАВЛЕННАЯ) =====
def handle_user_in_chat(message):
    ...
    allowed_types = chat_data.get('allowed_types', [])
...
def process_question(message):
    ...
    # Маскируем ссылки в тексте вопроса
    ...
    question_data = {
        ...
        'user_id': user_id,
        'username': username,
        'text': question_text,  # Оригинальный текст
        'masked_text': masked_text,  # Текст с маскированными ссылками
        'url_count': url_count,  # Количество ссылок
...
def show_tasks(message):
    ...
    for question in pending_questions:
        # Используем маскированный текст для отображения
...
def notify_admin_about_question(question_id, question_data):
    # Используем маскированный текст
    ...
    if question_data.get('url_count', 0) > 0:
        notification += f"\n\n🔗 *Важно:* если хотите увидеть ссылку целиком, напишите /Full в ответе"
...
def handle_callbacks(call):
    if call.data.startswith('accept_chat_'):
        ...
        storage.pending_chat_setup[user_id] = True  # Помечаем как ожидающий настройки
        storage.save_data()
        ...
        bot.send_message(
            user_id,
            "💬 *Запрос принят!*\n\n"
            "✅ Администратор принял ваш запрос на переписку.\n\n"
            "⏳ *Подождите, пока администратор настраивает правила переписки...*\n"
            "⚙️ Это займет всего минуту!",
            parse_mode='Markdown'
        )
...
def ask_content_types_step3(message, temp_data):
    ...
    try:
        ...
        for part in selected_nums.replace(' ', ',').split(','):
    ...
    # Убираем из ожидания настройки
    if temp_data['user_id'] in storage.pending_chat_setup:
        del storage.pending_chat_setup[temp_data['user_id']]
...
def process_admin_answer(message, question_id):
    ...
    # Для текстовых сообщений проверяем [Имя] в начале
    if message.content_type == 'text':
        text = message.text
...
if __name__ == '__main__':
    ...
    print(f"💬 Ожидают настройки чата: {len(storage.pending_chat_setup)}")
    print("=" * 50)
