...
def show_admin_help(message):
    help_text = (
        ...
        "*Команда /full:*\n"
        "• Можно использовать как прямую команду\n"
...
def full_command(message):
    ...
    # Проверяем формат /full #2 или /full 2
...
def handle_callbacks(call):
    if call.data.startswith('accept_chat_'):
    ...
    elif call.data.startswith('ask_links_'):
        # Обработка выбора разрешения ссылок
        parts = call.data.replace('ask_links_', '').split('_')
        user_id = int(parts[0])
        question_id = int(parts[1])
        allow_links = parts[2] == 'yes'
        ...
        complete_chat_setup(user_id, question_id)
        bot.answer_callback_query(call.id, f"✅ Ссылки {'разрешены' if allow_links else 'запрещены'}")
...
def ask_admin_name_step(message, user_id, question_id):
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton('✅ Да', callback_data=f'ask_links_{user_id}_{question_id}_yes'),
        types.InlineKeyboardButton('❌ Нет', callback_data=f'ask_links_{user_id}_{question_id}_no')
    )
...
def complete_chat_setup(user_id, question_id):
    """Завершает настройку чата после выбора разрешения ссылок"""
    chat_data = storage.active_chats.get(user_id)
    if not chat_data:
        # Создаем чат
        storage.active_chats[user_id] = {
            ...
            'admin_name': chat_data.get('admin_name', 'Администратор'),
