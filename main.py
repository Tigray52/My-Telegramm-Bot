...
def show_admin_help(message):
    help_text = (
        ...
        "• Нажмите кнопку, затем введите ответ\n"
        "• Ответ будет показан с заголовком 'Ответ на: [текст сообщения]'\n\n"
...
def end_chat(user_id, reason="normal"):
    ...
    if user_id in storage.active_chats:
        ...
        if reason != "ban" and storage.is_banned(user_id) is not True:
            try:
...
def handle_admin_actions(message):
    ...
    if pending_reply and message.content_type == 'text' and not message.text.startswith('/'):
        ...
        reply_text = f"↪️ *Ответ на:* {reply_to_text}\n_________________\n{message.text}"
    ...
    try:
        ...
        if urls and not allow_links:
            ...
            markup.add(
                ...
                types.InlineKeyboardButton('💬 Ответить', callback_data=f'reply_to_msg_{user_id}_{text[:50]}')
            )
...
def handle_admin_to_user(message):
    ...
    try:
        if message.content_type == 'text':
            ...
            markup = types.InlineKeyboardMarkup()
            markup.add(
                types.InlineKeyboardButton('💬 Ответить', callback_data=f'reply_to_msg_admin_{message.text[:50]}')
            )
...
def handle_callbacks(call):
    if call.data.startswith('view_violation_'):
        ...
        if len(parts) == 2:
            user_id_str = parts[0]
            reply_to_text = parts[1].replace('_', ' ')
...
if __name__ == '__main__':
    ...
    # Запускаем поток для проверки истекших банов
    ban_check_thread = threading.Thread(target=check_ban_expirations, daemon=True)
