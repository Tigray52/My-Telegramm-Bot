...
def show_admin_help(message):
    help_text = (
        ...
        "*Кнопки в меню:*\n"
        "📨 Задать вопрос - Для пользователей\n"
        "💬 Прямая переписка - Запрос чата\n"
        "ℹ️ Помощь - Эта справка\n\n"
        
        "*Примеры:*\n"
        "`/ban 123456789`\n"
        "`/message [123456789] Привет!`\n"
        "`/message [123456789] {false} Без рамок`"
    )
...
def stop_command(message):
    """Команда /stop для завершения чата"""
    if not is_admin(message.from_user.id):
        return
...
def handle_all_messages(message):
    """Обработка всех текстовых сообщений (кнопки меню)"""
    user_id = message.from_user.id
    
    # Если пользователь в бане
    if user_id in storage.banned_users:
        return
    
    # Если админ в обычном режиме
    ...
    if message.text in ['📨 Задать вопрос', '💬 Прямая переписка', 'ℹ️ Помощь']:
        ...
        # Если это не команда и не кнопка, игнорируем
        bot.send_message(user_id, "Используйте кнопки меню или команды.")
...
def handle_admin_actions(message):
    ...
    if message.text in ['📋 Задачи (/tasks)', '💬 Активные чаты', '🚫 Бан-лист', '🔄 Обновить']:
        ...
        # Если админ пишет сообщение - пересылаем в активный чат
