...
ADMIN_IDS = {MAIN_ADMIN_ID}  # Множество ID администраторов
        ...
        with self.lock:
            ...
            if not cursor.fetchone():
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?)
                ''', (MAIN_ADMIN_ID, 'main_admin', 'Main Admin', MAIN_ADMIN_ID, datetime.now()))
...
class Storage:
    ...
    def load_admins(self):
        ...
        for admin in admins:
            self.cache['admins'].add(admin['telegram_id'])
    ...
    def remove_admin(self, telegram_id):
        ...
        deleted = self.db.remove_admin(telegram_id)
        if deleted:
            self.cache['admins'].discard(telegram_id)
        return deleted, "Успешно удален" if deleted else "Администратор не найден"
...
def adminoff_command(message):
    ...
    if user_id == MAIN_ADMIN_ID:
        ...
        bot.send_message(
            ...
            f"пытался снять вас как администратора."
        )
        return
    ...
    if deleted:
        ...
        try:
            bot.send_message(
                ...
                f"Снял: {message.from_user.first_name}"
            )
        except:
            pass
    else:
...
def unmute_command(message):
    ...
    if mute_type == 'chat':
        ...
        if storage.unmute_user_chat(user_id):
            ...
            bot.send_message(message.chat.id, f"❌ Ошибка при снятии мута с пользователя `{user_id}`")
...
def ask_chat_limit_step(message, user_id, admin_name, allow_links):
    ...
    storage.start_chat(user_id, message.from_user.id, username, admin_name, allow_links, limit)
