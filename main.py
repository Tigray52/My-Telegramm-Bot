...
def admin_command(message):
    ...
    if success:
        ...
        try:
            ...
            # Отправляем панель админа новому администратору
            admin_panel(types.Message(message_id=message.message_id + 1, 
                                     date=message.date,
            ...
            print(f"Ошибка отправки панели новому админу: {e}")
    else:
...
def ask_chat_limit_step(message, user_id, admin_name, allow_links):
    ...
    if chat_id:
        # Сначала отправляем информацию о начале чата пользователю
        escaped_admin_name = escape_markdown(admin_name)
        ...
        # Затем отправляем автоприветственные сообщения (если есть)
