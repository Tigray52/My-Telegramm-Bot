...
class Storage:
    def __init__(self):
        ...
        self.chat_settings = {}  # {user_id: {'allow_links': True/False, 'max_msg_length': 400, 'max_msg_count': 15}}
        self.answer_counts = {}  # {question_id: count}
        self.user_message_counts = {}  # {user_id: {'count': X, 'start_time': timestamp}} для подсчета сообщений в чате
        self.last_chat_messages = {}  # {user_id: 'текст последнего сообщения с ссылкой'}
        self.load_data()
        ...
        try:
            if os.path.exists('storage.json'):
                with open('storage.json', 'r', encoding='utf-8') as f:
                    ...
                    self.last_chat_messages = data.get('last_chat_messages', {})
    ...
    def save_data(self):
        data = {
            ...
            'last_chat_messages': self.last_chat_messages
        }
        try:
    ...
    def increment_user_message_count(self, user_id):
        """Увеличивает счетчик сообщений пользователя в чате"""
        if user_id not in self.user_message_counts:
            self.user_message_counts[user_id] = {'count': 1, 'start_time': time.time()}
        else:
            self.user_message_counts[user_id]['count'] += 1
        self.save_data()
    
    def get_user_message_count(self, user_id):
        """Возвращает количество сообщений пользователя в текущем чате"""
        if user_id not in self.user_message_counts:
            return 0
        return self.user_message_counts[user_id].get('count', 0)
    
    def reset_user_message_count(self, user_id):
        """Сбрасывает счетчик сообщений пользователя"""
        if user_id in self.user_message_counts:
            del self.user_message_counts[user_id]
            self.save_data()
    
    def save_last_chat_message(self, user_id, message_text):
        """Сохраняет последнее сообщение пользователя в чате"""
        self.last_chat_messages[user_id] = message_text
        self.save_data()
    
    def get_last_chat_message(self, user_id):
        """Возвращает последнее сообщение пользователя в чате"""
        return self.last_chat_messages.get(user_id, '')

storage = Storage()

# Константы
CHAT_MESSAGE_LIMIT_DEFAULT = 400
MAX_MSG_COUNT_DEFAULT = 15
QUESTION_LIMIT = 400
...
def show_user_help(message):
    help_text = (
        ...
        "*💬 В чате:*\n"
        "• Длина сообщения ограничена настройками\n"
...
def show_admin_help(message):
    help_text = (
        ...
        "• /full - Показать полный текст диалога\n\n"
        ...
        "• Или после нажатия кнопки 'Полностью'\n\n"
        
        "*Настройки чата:*\n"
        "• Длина сообщений: 30-500 символов (по умолч. 400)\n"
        "• Макс. сообщений: 5-150 (по умолч. 15)\n\n"
...
def show_full_question_text(admin_id, question_id):
    ...
    full_text = f"📨 *Полный текст последнего диалога*\n\n"
    
    # Форматируем ID для копирования
    user_id_display = f"`{question['user_id']}`"
    username_display = question['username']
    
    full_text += f"👤 {username_display} ({user_id_display})\n"
    full_text += f"⏰ {question['time']} | {question['date']}\n\n"
    
    # Показываем оригинальный текст без маскировки
    ...
    # Продолжаем процесс ответа, если администратор ожидает
    if admin_id in storage.admin_pending_answers:
...
def handle_user_in_chat(message):
    ...
    allow_links = chat_settings.get('allow_links', True)
    max_msg_length = chat_settings.get('max_msg_length', CHAT_MESSAGE_LIMIT_DEFAULT)
    max_msg_count = chat_settings.get('max_msg_count', MAX_MSG_COUNT_DEFAULT)
    
    # Проверяем лимит сообщений
    current_msg_count = storage.get_user_message_count(user_id) + 1
    
    if current_msg_count > max_msg_count:
        # Достигнут лимит сообщений
        end_chat(user_id, "message_limit_reached")
        bot.send_message(user_id, f"⏹ Переписка завершена. Достигнут лимит в {max_msg_count} сообщений.")
        bot.send_message(ADMIN_ID, f"⏹ Чат с {chat_data['user_name']} завершен (достигнут лимит {max_msg_count} сообщений).")
        return
    ...
    try:
        if message.content_type == 'text':
            ...
            # Сохраняем сообщение
            storage.save_last_chat_message(user_id, text)
            
            # Проверяем длину
            if len(text) > max_msg_length:
                bot.send_message(user_id, f"⚠️ Сообщение слишком длинное ({len(text)}/{max_msg_length} символов)")
                return
            ...
            if urls and not allow_links:
                ...
                markup.add(
                    ...
                    types.InlineKeyboardButton('Полностью', callback_data=f'full_{chat_data["question_id"]}')
                )
        ...
        storage.increment_user_message_count(user_id)
...
def handle_admin_to_user(message):
    ...
    try:
        if message.content_type == 'text':
            if len(message.text.strip()) > max_msg_length:
                bot.send_message(ADMIN_ID, f"⚠️ Сообщение слишком длинное ({len(message.text)}/{max_msg_length} символов)")
                return
...
def show_tasks(message):
    ...
    for question in pending_questions:
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton('Полностью', callback_data=f'full_{question["id"]}'),
...
def show_active_chats(message):
    ...
    for user_id, chat_data in storage.active_chats.items():
        if chat_data['admin_id'] == ADMIN_ID:
            ...
            max_msg_length = chat_settings.get('max_msg_length', CHAT_MESSAGE_LIMIT_DEFAULT)
            max_msg_count = chat_settings.get('max_msg_count', MAX_MSG_COUNT_DEFAULT)
            current_count = storage.get_user_message_count(user_id)
            
            text += f"👤 {chat_data['user_name']}\n"
            text += f"ID: `{user_id}`\n"
            text += f"Имя админа: {chat_data['admin_name']}\n"
            text += f"Сообщений: {current_count}/{max_msg_count}\n"
            text += f"Длина: до {max_msg_length} симв.\n"
            text += f"Ссылки: {'✅ Разрешены' if chat_settings.get('allow_links', True) else '❌ Запрещены'}\n\n"
...
def notify_admin_about_question(question_id, question_data):
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton('Полностью', callback_data=f'full_{question_id}'),
    ...
    if question_data.get('url_count', 0) > 0:
        notification += f"\n\nℹ️ *Нажмите кнопку Полностью чтобы посмотреть раскрытые ссылки*"
...
def handle_callbacks(call):
    if call.data.startswith('accept_chat_'):
        ...
        # Обработка кликабельной кнопки Полностью
...
def ask_admin_name_step(message, user_id, question_id):
    ...
    # Спрашиваем про настройку длины сообщений
    msg = bot.send_message(
        ADMIN_ID,
        f"✅ Имя сохранено: *{admin_name}*\n\n"
        f"*Настройте длину сообщений в чате:*\n\n"
        f"Введите число от *30* до *500* символов.\n"
        f"По умолчанию будет установлено *{CHAT_MESSAGE_LIMIT_DEFAULT}* символов.\n\n"
        f"Пример: `400`",
        parse_mode='Markdown'
    )
    
    bot.register_next_step_handler(msg, ask_msg_length_step, user_id, question_id)

def ask_msg_length_step(message, user_id, question_id):
    text = message.text.strip()
    
    # Определяем длину сообщений
    max_msg_length = CHAT_MESSAGE_LIMIT_DEFAULT
    
    if text.isdigit():
        length = int(text)
        if 30 <= length <= 500:
            max_msg_length = length
            confirmation = f"✅ Установлена длина: *{max_msg_length}* символов"
        else:
            confirmation = f"❌ Число должно быть от 30 до 500. Установлено по умолчанию: *{max_msg_length}*"
    else:
        confirmation = f"❌ Неверный ввод. Установлено по умолчанию: *{max_msg_length}*"
    
    # Сохраняем длину сообщений
    if user_id not in storage.chat_settings:
        storage.chat_settings[user_id] = {}
    storage.chat_settings[user_id]['max_msg_length'] = max_msg_length
    
    # Спрашиваем про максимальное количество сообщений
    msg = bot.send_message(
        ADMIN_ID,
        f"{confirmation}\n\n"
        f"*Настройте максимальное количество сообщений:*\n\n"
        f"Введите число от *5* до *150* сообщений.\n"
        f"После достижения лимита чат автоматически завершится.\n"
        f"По умолчанию будет установлено *{MAX_MSG_COUNT_DEFAULT}* сообщений.\n\n"
        f"Пример: `15`",
        parse_mode='Markdown'
    )
    
    bot.register_next_step_handler(msg, ask_msg_count_step, user_id, question_id, max_msg_length)

def ask_msg_count_step(message, user_id, question_id, max_msg_length):
    text = message.text.strip()
    
    # Определяем максимальное количество сообщений
    max_msg_count = MAX_MSG_COUNT_DEFAULT
    
    if text.isdigit():
        count = int(text)
        if 5 <= count <= 150:
            max_msg_count = count
            confirmation = f"✅ Установлено: *{max_msg_count}* сообщений"
        else:
            confirmation = f"❌ Число должно быть от 5 до 150. Установлено по умолчанию: *{max_msg_count}*"
    else:
        confirmation = f"❌ Неверный ввод. Установлено по умолчанию: *{max_msg_count}*"
    
    # Сохраняем максимальное количество сообщений
    if user_id not in storage.chat_settings:
        storage.chat_settings[user_id] = {}
    storage.chat_settings[user_id]['max_msg_count'] = max_msg_count
    ...
    bot.register_next_step_handler(msg, ask_links_step, user_id, question_id, max_msg_length, max_msg_count)

def ask_links_step(message, user_id, question_id, max_msg_length, max_msg_count):
    ...
    complete_chat_setup(user_id, question_id, max_msg_length, max_msg_count, confirmation)

def complete_chat_setup(user_id, question_id, max_msg_length, max_msg_count, confirmation):
    ...
    bot.send_message(
        ...
        f"📏 Длина сообщений: до *{max_msg_length}* символов\n"
        f"📊 Макс. сообщений: *{max_msg_count}*\n"
    ...
    bot.send_message(
        ...
        f"💬 *Чат начат!*\n\n"
        f"{confirmation}\n"
        f"📏 Длина сообщений: *{max_msg_length}* символов\n"
        f"📊 Макс. сообщений: *{max_msg_count}*\n\n"
...
def end_chat(user_id, reason="normal"):
    ...
    if user_id in storage.active_chats:
        ...
        if reason == "user_used_command":
            ...
            bot.send_message(admin_id, f"⏹ Чат с {user_name} завершен (достигнут лимит сообщений)")
        elif reason == "ban":
        ...
        storage.reset_user_message_count(user_id)
