...
class Storage:
    def __init__(self):
        ...
        self.user_cooldowns = {}
        self.pending_chat_setup = {}
        self.admin_pending_answers = {}  # {admin_id: question_id} - для обработки ответов с /Full
        self.load_data()
...
# ===== КОМАНДА /FULL (РАБОТАЕТ В ЛЮБОМ СЦЕНАРИИ) =====
@bot.message_handler(commands=['full', 'Full'])
def full_command(message):
    if not is_admin(message.from_user.id):
        return
    
    # Если это прямой вызов команды (не ответ)
    if not message.reply_to_message:
        # Проверяем, есть ли активный вопрос для ответа
        if ADMIN_ID in storage.admin_pending_answers:
            ...
            bot.send_message(ADMIN_ID, "❌ Используйте команду как ответ на вопрос или после нажатия кнопки 'Ответить'.")
        return
    ...
    # Ищем вопрос по ID в тексте сообщения
    match = re.search(r'#(\d+)', reply_msg.text or reply_msg.caption or '')
    if match:
        question_id = int(match.group(1))
    else:
        # Альтернативный поиск по контенту
        for qid, question in storage.questions.items():
            if question.get('has_media') and reply_msg.caption:
                if question.get('masked_text', '') in reply_msg.caption:
                    question_id = qid
                    break
            elif reply_msg.text and question.get('masked_text', '') in reply_msg.text:
                question_id = qid
                break
...
def show_full_question_text(admin_id, question_id):
    ...
    # Если был ожидающий ответ, продолжаем процесс
    if admin_id in storage.admin_pending_answers:
...
def handle_callbacks(call):
    if call.data.startswith('accept_chat_'):
        ...
        storage.pending_chat_setup[user_id] = True
        storage.save_data()
