...
def ban_command(message):
    ...
    # Очищаем сохраненное нарушение если есть
...
# ===== НОВАЯ ФУНКЦИЯ: ПОКАЗ ПОЛНОГО СООБЩЕНИЯ ПРИ НАРУШЕНИИ =====
def show_full_violation_message(admin_id, user_id):
    ...
    # Форматируем сообщение в указанном стиле
    message_text = (
        f"👤 {username} (`{user_id}`)\n"
    ...
    try:
        if message.content_type == 'text':
            ...
            if len(text) > CHAT_MESSAGE_LIMIT:
                bot.send_message(user_id, f"⚠️ Сообщение слишком длинное ({len(text)}/{CHAT_MESSAGE_LIMIT} символов)")
                return
...
def handle_callbacks(call):
    # Обработка просмотра нарушения со ссылками
...
def ask_links_step(message, user_id, question_id):
    ...
    complete_chat_setup(user_id, question_id, confirmation)

def complete_chat_setup(user_id, question_id, confirmation):
...
def process_admin_answer(message, question_id):
    ...
    admin_name = None
    answer_caption = None # Переменная для хранения текста из медиафайла
    ...
    if message.content_type == 'text':
        ...
        # Основное исправление: проверяем наличие caption, но не требуем его
        ...
        # Если caption нет, ответ будет без текста, только медиа
        else:
            answer_caption = None
    ...
    try:
        ...
        if admin_name:
            header = f"📩 *Ответ на ваш вопрос #{question_id}:*\n\n*Вопрос:* {question_preview}\n\n*Ответ от \"{admin_name}\" (администратора):*"
        else:
        ...
        if message.content_type == 'text':
            ...
            final_caption = f"{header}\n\n{answer_caption}" if answer_caption else header
            bot.send_voice(user_id, message.voice.file_id, caption=final_caption, parse_mode='Markdown')
            
        elif message.content_type == 'photo':
            final_caption = f"{header}\n\n{answer_caption}" if answer_caption else header
            bot.send_photo(user_id, message.photo[-1].file_id, caption=final_caption, parse_mode='Markdown')
            
        elif message.content_type == 'animation':
            final_caption = f"{header}\n\n{answer_caption}" if answer_caption else header
            bot.send_animation(user_id, message.animation.file_id, caption=final_caption, parse_mode='Markdown')
            
        elif message.content_type == 'document':
            final_caption = f"{header}\n\n{answer_caption}" if answer_caption else header
            bot.send_document(user_id, message.document.file_id, caption=final_caption, parse_mode='Markdown')
            
        elif message.content_type == 'video_note':
            bot.send_video_note(user_id, message.video_note.file_id)
            bot.send_message(user_id, f"{header}\n\n[Кружочек]", parse_mode='Markdown')
        
        # Обновляем статус вопроса и счетчик
        ...
        if remaining > 0:
            bot.send_message(ADMIN_ID, f"✅ Ответ #{question_id} отправлен {question['username']}\n\nℹ️ Можно отправить еще {remaining} ответов на этот вопрос.")
        else:
            bot.send_message(ADMIN_ID, f"✅ Ответ #{question_id} отправлен {question['username']}\n\nℹ️ Достигнут лимит ответов на этот вопрос ({MAX_ANSWERS_PER_QUESTION}).")
        
    except Exception as e:
        bot.send_message(ADMIN_ID, f"❌ Ошибка при отправке: {str(e)}")
