...
def process_admin_answer(message, question_id):
    ...
    if message.content_type == 'text':
        ...
        # Для медиа-сообщений: обработка caption если есть
        if hasattr(message, 'caption') and message.caption:
            ...
            if name_match:
                ...
                answer_text = text.strip() if text else None
    ...
    try:
        ...
        # Подготовка текста для медиафайлов
        caption_text = ""
        if answer_text:
        ...
        if message.content_type == 'text':
            ...
            bot.send_voice(user_id, message.voice.file_id, caption=caption_text, parse_mode='Markdown')
            
        elif message.content_type == 'photo':
            # Берем самое большое фото
            file_id = message.photo[-1].file_id
            bot.send_photo(user_id, file_id, caption=caption_text, parse_mode='Markdown')
            
        elif message.content_type == 'animation':
            bot.send_animation(user_id, message.animation.file_id, caption=caption_text, parse_mode='Markdown')
            
        elif message.content_type == 'document':
            bot.send_document(user_id, message.document.file_id, caption=caption_text, parse_mode='Markdown')
            ...
            bot.send_video(user_id, message.video.file_id, caption=caption_text, parse_mode='Markdown')
        
        elif message.content_type == 'audio':
            bot.send_audio(user_id, message.audio.file_id, caption=caption_text, parse_mode='Markdown')
