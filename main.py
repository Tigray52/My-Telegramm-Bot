...
class Storage:
    def __init__(self):
        self.questions = {}
        self.active_chats = {}
        self.chat_history = {}  # {user_id: [сообщения]} - история чатов
    ...
    def add_chat_message(self, user_id, sender, message_type, content, timestamp=None):
    ...
    def get_chat_history(self, user_id):
        """Возвращает историю чата для пользователя"""
...
def mask_url(url):
    """Маскирует URL, оставляя первую букву домена и точку (режим: h•••••••••••••.ru)"""
    try:
...
def show_admin_help(message):
    help_text = (
        ...
        "*При завершении чата:*\n"
        "Бот предложит скачать историю переписки в формате TXT"
    )
...
def stop_command(message):
    ...
    if is_admin(user_id):
        ...
        if active_user_id:
            # Завершаем чат с опцией сохранения истории
            end_chat_with_history_option(active_user_id, ADMIN_ID, initiator="admin")
        else:
    ...
    if is_user_in_chat(user_id):
        ...
        if chat_data:
            # Завершаем чат с опцией сохранения истории для админа
            end_chat_with_history_option(user_id, chat_data['admin_id'], initiator="user")
        return
...
def ban_command(message):
    ...
    if user_id in storage.active_chats:
        end_chat_with_history_option(user_id, storage.active_chats[user_id]['admin_id'], initiator="ban")
...
# ===== ОБРАБОТКА СООБЩЕНИЙ В ЧАТЕ (С СОХРАНЕНИЕМ ИСТОРИИ) =====
...
# ===== ФУНКЦИИ ДЛЯ СОХРАНЕНИЯ ИСТОРИИ ЧАТА =====
def create_chat_history_file(user_id):
    """Создает TXT файл с историей чата"""
    ...
    filename = f"chat_history_{user_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    file_content = f"История переписки\n"
    file_content += f"=================\n\n"
    
    file_content += f"Пользователь: {chat_data.get('user_name', 'Неизвестно')}\n"
    file_content += f"ID пользователя: {user_id}\n"
    file_content += f"Администратор: {chat_data.get('admin_name', 'Неизвестно')}\n"
    file_content += f"Начало чата: {chat_data.get('start_time', 'Неизвестно')}\n"
    file_content += f"Конец чата: {datetime.now().isoformat()}\n\n"
    file_content += f"Сообщения:\n"
    file_content += f"----------\n\n"
    ...
    for i, msg in enumerate(history, 1):
        ...
        file_content += f"{i}. [{timestamp}] {sender} ({msg_type}):\n"
        file_content += f"   {content}\n\n"
    
    file_content += f"\n=================\n"
    file_content += f"Всего сообщений: {len(history)}\n"
    file_content += f"Файл создан: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
...
def end_chat_with_history_option(user_id, admin_id, initiator="admin"):
    """Завершает чат с предложением сохранить историю"""
    ...
    if initiator == "admin":
        initiator_name = f"администратор {admin_name}"
        # Уведомляем пользователя
        bot.send_message(user_id, f"⏹ Переписка завершена администратором.")
    elif initiator == "user":
        initiator_name = f"пользователь {user_name}"
        # Уведомляем администратора о том, что пользователь завершил
        bot.send_message(admin_id, f"⏹ Пользователь {user_name} завершил переписку.")
    elif initiator == "ban":
        initiator_name = f"система (бан)"
    else:
        initiator_name = "система"
    
    # Уведомляем администратора о завершении
    notification = f"⏹ Чат с {user_name} завершен ({initiator_name})."
    
    # Создаем инлайн-кнопки для выбора сохранения истории
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton('📥 Скачать историю (TXT)', callback_data=f'save_history_{user_id}'),
        types.InlineKeyboardButton('❌ Без истории', callback_data=f'close_chat_{user_id}')
    )
    
    bot.send_message(admin_id, notification, reply_markup=markup)
    
    # Не удаляем чат сразу, ждем выбора администратора
    # Чат будет удален после выбора в callback

@bot.callback_query_handler(func=lambda call: call.data.startswith(('save_history_', 'close_chat_')))
def handle_chat_history_choice(call):
    """Обрабатывает выбор администратора по сохранению истории"""
    if call.data.startswith('save_history_'):
        user_id = int(call.data.replace('save_history_', ''))
        
        if user_id not in storage.active_chats:
            bot.answer_callback_query(call.id, "❌ Чат уже закрыт")
            return
        
        # Создаем файл с историей
        result = create_chat_history_file(user_id)
        ...
        if result:
            ...
            try:
                ...
                with open(file_path, 'rb') as f:
                    bot.send_document(ADMIN_ID, f, caption=f"📋 История переписки с {storage.active_chats[user_id]['user_name']}")
                
                bot.answer_callback_query(call.id, "✅ История сохранена")
            ...
            bot.answer_callback_query(call.id, "❌ История чата пуста")
        
        # Закрываем чат
        close_chat(user_id)
        
    elif call.data.startswith('close_chat_'):
        user_id = int(call.data.replace('close_chat_', ''))
        
        if user_id not in storage.active_chats:
            bot.answer_callback_query(call.id, "❌ Чат уже закрыт")
            return
        
        # Просто закрываем чат без сохранения истории
        close_chat(user_id)
        bot.answer_callback_query(call.id, "✅ Чат закрыт")

def close_chat(user_id):
    """Закрывает чат и очищает историю"""
    if user_id in storage.active_chats:
        del storage.active_chats[user_id]
    
    # Очищаем историю (или можно оставить для архива)
