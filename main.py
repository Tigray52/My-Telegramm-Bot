...
class Database:
    ...
    def update_user_mute(self, user_id, is_muted, reason=None, mute_until=None):
    ...
    def get_statistics(self):
        ...
        with self.lock:
            ...
            # Муты
            cursor.execute('SELECT COUNT(*) FROM users WHERE is_muted = TRUE')
...
class Storage:
    ...
    def is_muted(self, telegram_id):
        ...
        if user['mute_until']:
            ...
            if datetime.now() > mute_until:
                # Мут истек
                self.db.update_user_mute(user['id'], False)
                return False
    ...
    def mute_user(self, telegram_id, duration_seconds=0, reason="Нарушение правил"):
        ...
        self.db.update_user_mute(user['id'], True, reason, mute_until)
        return True
...
def check_ban_expirations():
    ...
    while True:
        try:
            # Проверяем баны
            for user_id, user_data in storage.db.get_all_users():
            ...
            # Проверяем муты
            for user_id, user_data in storage.db.get_all_users():
                if user_data['is_muted'] and user_data['mute_until']:
                    mute_until = datetime.fromisoformat(user_data['mute_until'])
                    if datetime.now() > mute_until:
                        storage.db.update_user_mute(user_data['id'], False)
                        ...
                        try:
                            bot.send_message(
                                ...
                                f"Причина мута: {user_data['mute_reason']}"
                            )
                        except:
                            pass
...
def unmute_command(message):
    ...
    if storage.unmute_user(user_id):
        ...
        bot.send_message(ADMIN_ID, f"❌ Пользователь `{user_id}` не найден или не заглушен.")
    ...
    if message.text == '📨 Задать вопрос':
        ...
        if storage.is_muted(user_id):
            ...
            bot.send_message(
                ...
                f"Причина: {user_data['mute_reason']}\n"
                f"Мут: {mute_time}\n\n"
...
def process_mute_with_reason(message, user_id):
    ...
    bot.send_message(ADMIN_ID, f"🔇 Пользователь `{user_id}` ({username}) заглушен на {duration_text}.\nПричина: {reason}")
    ...
    try:
        ...
        bot.send_message(
            ...
            f"Причина: {reason}\n"
            f"Срок: {mute_time}\n\n"
            f"Вы поPl-emeremment можете задавать вопросы через раздел 📨 Задать вопрос."
        )
    except:
        pass
...
if __name__ == '__main__':
    ...
    print(f"🔇 Активных мутов: {stats['mutes']}")
