...
def escape_markdown_v2(text):
    ...
    # В Markdown V2 нужно экранировать другие символы
    escape_chars = r'_*[]()~`>#+-=|{}.!'
    return re.sub(f'([{re.escape(escape_chars)}])', r'\\\1', text)

def escape_markdown(text):
    """Экранирует специальные символы Markdown (старая версия для совместимости)"""
    if not text:
        return text
    
    # Экранируем только символы, которые могут вызвать проблемы в Markdown
    # Но оставляем точки, запятые и другие обычные символы
    escape_chars = r'_*[]()~`>#+-=|{}!'
    escaped = re.sub(f'([{re.escape(escape_chars)}])', r'\\\1', text)
    
    # Убираем лишнее экранирование для точек в конце предложений
...
def show_admin_help(message):
    help_text = (
        ...
        "`/mute question 123456789 1w6d нарушение правил`\n\n"
        
        "*Формат времени:*\n"
        "• y - годы, mon - месяцы, w - недели\n"
        "• d - дни, h - часы, m - минуты, s - секунды\n"
        "• Можно комбинировать: 1y6mon2w3d5h10m30s\n\n"
        ...
        "*Команда /stauser:*\n"
        "`/stauser 123456789` - статистика пользователя\n\n"
...
def stop_command(message):
    ...
    if reason:
        ...
        # Сообщение уже отправляется в функции end_chat_with_reason
    else:
...
def end_chat(user_id, reason="normal"):
    ...
    try:
        # Отправляем одно сообщение админу с ID пользователя
...
def end_chat_with_reason(user_id, reason):
    ...
    # Уведомляем админа ОДНИМ сообщением
    try:
...
def mute_command(message):
    """Универсальная команда для мута"""
    if not is_admin(message.from_user.id):
        return
    ...
    mute_type = parts[1].lower()  # 'chat' или 'question'
    user_id_str = parts[2]
    
    if mute_type not in ['chat', 'question']:
        bot.send_message(ADMIN_ID, "❌ Тип мута должен быть 'chat' или 'question'")
        return
    ...
    duration_text = "навсегда" if duration_seconds == 0 else format_duration(duration_seconds)
    bot.send_message(ADMIN_ID, f"✅ Пользователь `{user_id}` заглушен {mute_text} на {duration_text}.\nПричина: {reason}")
...
def show_full_question_text(admin_id, question_id):
    """Показывает полный текст вопроса с группировкой одинаковых ссылок"""
    question = storage.get_question(question_id)
    if not question:
    ...
    # Находим и группируем ссылки (без учета регистра)
    urls = re.findall(r'(?i)https?://[^\s<>"]+|www\.[^\s<>"]+\.[^\s<>"]+', question['text'])
    
    if urls:
        # Группируем одинаковые ссылки (приводим к нижнему регистру для сравнения)
        ...
        for url in urls:
            ...
            if normalized_url not in url_examples:
                url_examples[normalized_url] = url  # Сохраняем оригинальный регистр для примера
...
def clients_command(message):
    ...
    limit = 20  # По умолчанию 20 пользователей
    ...
    if len(parts) >= 2:
        ...
        if param == 'all':
            ...
            limit = min(int(param), 100)  # Максимум 100 пользователей
        else:
            bot.send_message(ADMIN_ID, 
                           "Используйте:\n"
                           "`/clients` - 20 пользователей\n"
    ...
    if not users:
        bot.send_message(ADMIN_ID, "📭 Нет пользователей в базе данных.")
        return
...
def stauser_command(message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        bot.send_message(ADMIN_ID, "Используйте: `/stauser ID`", parse_mode='Markdown')
        return
...
def handle_user_in_chat(message):
    ...
    try:
        ...
        # МЕНЬШЕ ЭКРАНИРОВАНИЯ для обычных сообщений
        escaped_sender = escape_markdown(sender)
        
        # Отправляем сообщение с минимальным экранированием
        sent_msg = bot.send_message(
            ADMIN_ID,
            f"👤 *{escaped_sender}:*\n{text[:500]}",
            parse_mode='Markdown',
...
def handle_admin_to_user(message):
    ...
    try:
        if message.content_type == 'text':
            # МЕНЬШЕ ЭКРАНИРОВАНИЯ для админских сообщений
...
def show_mutes_questions(message):
    """Показывает пользователей заглушенных в вопросах"""
    muted_users = storage.get_muted_questions_users()
    
    if not muted_users:
        bot.send_message(ADMIN_ID, "✅ Нет пользователей заглушенных в вопросах.")
        return
...
def show_mutes_chat(message):
    """Показывает пользователей заглушенных в переписке"""
    muted_users = storage.get_muted_chat_users()
    
    if not muted_users:
        bot.send_message(ADMIN_ID, "✅ Нет пользователей заглушенных в переписке.")
        return
