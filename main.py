...
def show_admin_help(message):
    help_text = (
        ...
        "• Или кликнуть на *Full* в сообщении\n\n"
...
# ===== КОМАНДА /FULL (НОВАЯ ВЕРСИЯ - ПОКАЗЫВАЕТ ПОЛНЫЙ ТЕКСТ ДИАЛОГА) =====
        ...
        if message.content_type == 'text':
            ...
            if urls and not allow_links:
                ...
                markup.add(
                    ...
                    types.InlineKeyboardButton('*Full*', callback_data=f'full_{chat_data["question_id"]}')
                )
...
def notify_admin_about_question(question_id, question_data):
    ...
    # Добавляем кнопку *Full* вместо /full#id
    buttons.append(types.InlineKeyboardButton('*Full*', callback_data=f'full_{question_id}'))
    ...
    # Форматируем текст уведомления по-новому
    notification = (
        f"📨 *Последний диалог #{question_id}*\n"
    ...
    if question_data.get('url_count', 0) > 0:
        notification += f"\n\nℹ️ *Нажмите кнопку *Full* чтобы посмотреть раскрытые ссылки*"
...
def handle_callbacks(call):
    if call.data.startswith('accept_chat_'):
        ...
        msg = bot.send_message(
            ...
            f"ℹ️ *Если нужно посмотреть полный текст со ссылками, нажмите кнопку *Full* выше*",
            parse_mode='Markdown'
        )
        ...
        # Обработка кликабельной кнопки *Full*
        question_id = int(call.data.replace('full_', ''))
        show_full_question_text(ADMIN_ID, question_id)
        bot.answer_callback_query(call.id)

# ===== НОВЫЕ ФУНКЦИИ НАСТРОЙКИ ЧАТА =====
